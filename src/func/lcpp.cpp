...
const double scale = 19.34236;

void parseFile(const std::string& inputPath, std::vector<double>& yawVals, std::vector<double>& pitchVals, int& maxIndex) {
    ...
    while (std::getline(inFile, line)) {
        ...
        if (std::regex_search(line, m, yawRegex)) {
            ...
            if (yawVals.size() <= static_cast<size_t>(idx)) yawVals.resize(idx + 1);
            yawVals[idx] = v;
            maxIndex = std::max(maxIndex, idx);
        } else if (std::regex_search(line, m, pitchRegex)) {
            int idx = std::stoi(m[1].str());
            double v = std::stod(m[2].str());
            if (pitchVals.size() <= static_cast<size_t>(idx)) pitchVals.resize(idx + 1);
            pitchVals[idx] = v;
            maxIndex = std::max(maxIndex, idx);
        }
    }
...
void writeOutput(const std::filesystem::path& outFile, const std::vector<double>& yawVals, const std::vector<double>& pitchVals, int maxIndex) {
    ...
    for (int i = 1; i <= maxIndex; ++i) {
        double yy = yawVals[i] * scale;
        double pp = pitchVals[i] * scale;
...
int run(int argc, char* argv[]) {
    ...
    try {
        ...
        writeOutput(outFile, yawVals, pitchVals, maxIndex);
